...
State size: 33 (information about position, rotation, velocity and angular velocities) \
Action size: 4 (Torque value for each join) \
Each action range: [-1,1] 

The environment is considered solved when the average score across all 20 agents reaches 30 for 100 episodes in a row.
...
In the command window, pass as a first argument the file path to the checkpoint. \
...
Then, from the command window, launch *watch_agent.py* file with the file path to the checkpoint as the first argument. \
